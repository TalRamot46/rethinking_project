...
%
%  Compile with:  pdflatex essay_part1.tex   (twice)
%==============================================================================
...
\author{By Tal Ramot, implemented with Claude AI}
...
\includegraphics[width=0.92\textwidth]{outputs/part1/fig3a_trace_centered.png}\\[6pt]
\textbf{\small (b) non-centered}\\[2pt]
\includegraphics[width=0.92\textwidth]{outputs/part1/fig3b_trace_noncentered.png}
...
drifting low around iteration $800$ and high around $900$. The non-centered
trace oscillates rapidly with no such drift. Slow excursions mean successive
draws carry nearly the same information, which is exactly what a low effective
sample size reports.}
\end{figure}
...
\centering
\textbf{\small (a) centered}\\[2pt]
\includegraphics[width=0.92\textwidth]{outputs/part1/fig4a_trank_centered.png}\\[6pt]
\textbf{\small (b) non-centered}\\[2pt]
\includegraphics[width=0.92\textwidth]{outputs/part1/fig4b_trank_noncentered.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.95\textwidth]{outputs/part1/fig5_population.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{outputs/part1/fig2_shrinkage.png}
...
\midrule
0.150 & $-268.0$ &  1 & $-267.8$ & $-265.5$ \\
0.194 & $-265.1$ &  2 & $-264.3$ & $-263.1$ \\
0.250 & $-262.3$ &  3 & $-259.8$ & $-260.0$ \\
0.323 & $-256.7$ &  6 & $-253.9$ & $-256.7$ \\
0.417 & $-252.9$ &  5 & $-249.3$ & $-253.9$ \\
\textbf{0.539} & $-249.0$ & 17 & $-243.9$ & $\mathbf{-252.1}$ \\
0.696 & $-247.7$ & 26 & $-240.7$ & $-252.4$ \\
\textbf{0.899} & $\mathbf{-247.2}$ & 30 & $-237.9$ & $-255.4$ \\
\textbf{1.161} & $-247.9$ & 45 & $\mathbf{-236.5}$ & $-261.9$ \\
1.499 & $-251.7$ & 62 & $-237.2$ & $-272.3$ \\
1.936 & $-258.9$ & 67 & $-239.6$ & $-286.7$ \\
2.500 & $-263.3$ & 96 & $-240.9$ & $-304.6$ \\
\bottomrule
\end{tabular}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{outputs/part1/fig1_sigma_agreement.png}
...
\textbf{The direct cross-validation curve peaks on the posterior mode.} A purely
predictive criterion, which knows nothing about priors, and a purely Bayesian
one, which never simulated a held-out data point, choose the same amount of
pooling. The multilevel model found by regularization what cross-validation
...
only to the nearest grid point. And the peak is \emph{flat} --- the direct elpd
differs by only $0.2$ between $\sigma = 0.539$ and $\sigma = 0.696$, so the
data do not distinguish sharply between nearby amounts of pooling. What the
curve does say sharply is that both ends are bad: pinning $\sigma$ at $0.15$ or
at $2.5$ costs more than $13$ and $50$ elpd respectively. The claim is
agreement, not identity.

The disagreement between the criteria is a result in its own right. PSIS and
WAIC both peak at noticeably \emph{larger} $\sigma$ --- that is, they recommend
\emph{less} pooling than is good for prediction. The reason is the same one that
made PSIS unreliable: both estimate the cost of a held-out point using a
posterior in which that point's own parameter was fitted to it. They therefore
undercount how much flexibility the model is using, and a model whose
flexibility is undercounted looks like it can afford more. The exact computation
has no such blind spot. It is a useful reminder that an information criterion is
an approximation, and that approximations have assumptions --- one observation
per cluster violates this one.
